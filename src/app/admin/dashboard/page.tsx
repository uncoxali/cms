...
export default function DashboardPage() {
  ...
  return (
    <Box>
      ...
      {/* Stat Cards */}
      <Grid container spacing={1.8} sx={{ mb: 2.5, alignItems: 'stretch' }}>
        {visibleStatCards.map(card => (
          <Grid size={{ xs: 12, sm: 6, md: 3 }} key={card.id}>
            <Paper sx={{
              ...
              '&:hover': { transform: 'translateY(-2px)', boxShadow: `0 8px 24px ${card.color}1F` },
            }}>
              <Box sx={{
              ...
              <Box sx={{ position: 'relative', zIndex: 1 }}>
                ...
                {loading ? (
                  ...
                  <Typography variant="h5" fontWeight={800} letterSpacing="-0.03em" sx={{ lineHeight: 1.1 }}>
                    {getStatValue(card.id)}
                  </Typography>
                )}
                <Typography variant="body2" color="text.secondary" fontSize={12} sx={{ mt: 0.5 }}>
                  {card.label}
                </Typography>
              </Box>
            </Paper>
          ...
          >
            ...
            {loading ? (
              ...
              <Box sx={{ py: 3, textAlign: 'center' }}>
                <Clock size={28} style={{ opacity: 0.2, marginBottom: 6 }} />
                    ...
                    <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography component="div" variant="body2" fontWeight={600} fontSize={13}>
                          {log.user}
                          <Typography component="span" color="text.secondary" fontSize={12} fontWeight={400}> {log.action} </Typography>
                          {log.collection && (
