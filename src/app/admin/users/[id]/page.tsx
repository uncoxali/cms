...
export default function UserEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load user data for edit mode (admins only)
  useEffect(() => {
    if (role === 'admin' && !isNew) {
      loadUser();
    }
  ...
  const handleSave = async (stay = false) => {
    ...
    try {
      ...
      if (isNew) {
        ...
        addNotification({
          title: 'User Created',
          message: `${form.firstName || form.email} has been created.`,
        });

        if (!form.password) {
          // Trigger mock invite flow
          try {
            await api.post('/flows/run/invite-user', { email: form.email });
          } catch {}
          addNotification({
            title: 'Invite Sent',
            message: `Invitation email sent to ${form.email}.`,
          });
        }
        ...
        if (originalData) {
          ...
          if ((roleMap[originalData.role_name] || originalData.role) !== form.roleId) {
            addLog({
              ...
              item: id,
              user: currentUser?.name || 'Admin',
              meta: { field: 'role', from: originalData.role_name, to: roleNameMap[form.roleId] },
            });
          }
          if (originalData.status !== form.status) {
            addLog({
              ...
              item: id,
              user: currentUser?.name || 'Admin',
              meta: { field: 'status', from: originalData.status, to: form.status },
            });
          }
          if (originalData.email !== form.email) {
            addLog({
              ...
              item: id,
              user: currentUser?.name || 'Admin',
              meta: { field: 'email', from: originalData.email, to: form.email },
            });
          }
        }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: `Password for ${userName} has been changed.`,
      });
  ...
  const openAvatarDialog = () => {
    ...
    if (!avatarFiles.length) loadAvatarFiles();
  };
  ...
  const handleSelectAvatar = async (file: any) => {
    ...
    setAvatarError(null);
    try {
      await api.patch(`/users/${id}`, { avatar: avatarPath });
  ...
  const handleClearAvatar = async () => {
    if (isNew || !form.avatar) return;
    setAvatarSaving(true);
    setAvatarError(null);
    try {
      await api.patch(`/users/${id}`, { avatar: null });
      updateField('avatar', '');
      if (currentUser && String(currentUser.id) === String(id)) {
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mt: 1 }}>
          ...
          <Button
            variant='contained'
            startIcon={isNew ? <Plus size={16} /> : <Save size={16} />}
      ...
      <Tabs
        ...
        sx={{ mb: 3, minHeight: 36, '& .MuiTab-root': { minHeight: 36, py: 0.5, fontSize: 13 } }}
      >
              ...
              <Paper sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 4 }}>
                  <Box sx={{ position: 'relative' }}>
                    ...
                    {!isNew && (
                      <Tooltip title='Choose avatar from File Library'>
                        <IconButton
                          size='small'
                ...
                <TextField
                  ...
                  slotProps={{
                    input: {
                      startAdornment: <Mail size={16} style={{ marginRight: 8, opacity: 0.4 }} />,
                    },
                  }}
                />
                        ...
                        <MenuItem key={r.id} value={r.id}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Shield
                              size={14}
                              color={
                                r.adminAccess ? '#e74c3c' : r.appAccess ? '#3498db' : '#7f8c8d'
                              }
                            />
                            {r.name}
                          </Box>
                    ...
                    >
                      {STATUS_OPTIONS.map((s) => (
                        <MenuItem key={s.value} value={s.value}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box
                              sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: s.color }}
                            />
                            {s.label}
                          </Box>
                        </MenuItem>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          <Box
                            sx={{
                              ...
                              height: 44,
                              borderRadius: '12px',
                              bgcolor: currentRole.adminAccess
                                ? 'rgba(231,76,60,0.12)'
                          ...
                          >
                            <Shield
                              size={22}
                              color={currentRole.adminAccess ? '#e74c3c' : '#3498db'}
                            />
                          </Box>
                          <Box>
                      ...
                      <Grid container spacing={2}>
                        <Grid size={{ xs: 6 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                            {currentRole.appAccess ? (
                              <ShieldCheck size={18} color={theme.palette.success.main} />
                            ) : (
                              <ShieldOff size={18} color={theme.palette.error.main} />
                            )}
                            <Box>
                ...
                <Paper sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <FormControlLabel
                      ...
                      label={form.twoFactorEnabled ? 'Enabled' : 'Disabled'}
                  ...
                  <Box sx={{ display: 'flex', gap: 2 }}>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        addNotification({
                          title: 'Email Sent',
                          message: `Password reset email sent to ${form.email}.`,
                        })
                      }
                      sx={{ borderColor: 'divider' }}
                    >
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Box>
                          ...
                          >
                            ...
                            {session.location} • {timeAgo(session.lastSeen)}
                          </Typography>
                        </Box>
                      </Box>
                      {!session.current && (
                        <IconButton
                          ...
                          onClick={() =>
                            addNotification({
                              title: 'Session Ended',
                              message: `Signed out from ${session.device}.`,
                            })
                          }
                        >
                          <LogOut size={16} />
              ...
              <Paper sx={{ p: 0 }}>
                ...
                {userLogs.length > 10 && (
                  <TablePagination
                    ...
                    page={activityPage}
                    onPageChange={(_, p) => setActivityPage(p)}
                    rowsPerPage={10}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Paper sx={{ p: 2.5 }}>
                ...
                {[
                  {
                    label: 'ID',
                    value: isNew ? 'Auto-generated' : id,
                    icon: <Database size={14} />,
                  },
                  {
                    label: 'Created',
                    value: originalData?.date_created
                      ? new Date(originalData.date_created).toLocaleDateString()
                      : isNew
                        ? 'On save'
                        : '—',
                    icon: <Clock size={14} />,
                  ...
                  {
                    label: 'Last Login',
                    value: originalData?.last_access ? timeAgo(originalData.last_access) : '—',
                    icon: <LogIn size={14} />,
                  },
                ].map((item) => (
                  <Box
              ...
              <Paper sx={{ p: 2.5 }}>
                ...
                <Typography variant='caption' color='text.secondary'>
                  ...
                  {form.status === 'suspended' &&
                    'This user account has been suspended and cannot log in.'}
                  {form.status === 'invited' &&
                    "This user has been invited but hasn't set up their account yet."}
                  {form.status === 'archived' &&
                    'This user account has been archived and is read-only.'}
                </Typography>
              </Paper>
                ...
                <Paper sx={{ p: 2.5 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                    <Shield
                      size={16}
                      color={
                        currentRole.adminAccess ? theme.palette.error.main : theme.palette.info.main
                      }
                    />
                    <Typography variant='body2' fontWeight={600}>
                      {currentRole.name}
      ...
      {!isNew && (
        ...
        >
          ...
          <DialogActions>
            ...
            <Button onClick={() => !avatarSaving && setAvatarDialogOpen(false)}>Close</Button>
          </DialogActions>
        </Dialog>
      )}
      ...
      >
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setResetPasswordOpen(false)} color='inherit'>
            Cancel
          </Button>
          <Button
            variant='contained'
