...
async function getEmailTemplates(req, res) {
    try {
        const templates = await (0, database_1.db)('email_templates').select('*').orderBy('date_created', 'desc');
        res.json({
...
async function getEmailTemplate(req, res) {
    try {
        const { id } = req.params;
        const template = await (0, database_1.db)('email_templates').where({ id }).first();
        if (!template)
...
async function createEmailTemplate(req, res) {
    try {
        ...
        await (0, database_1.db)('email_templates').insert({
            id,
            name,
            subject,
            body: emailBody || '',
        ...
        if (error.message?.includes('no such table')) {
            try {
                await database_1.db.schema.createTable('email_templates', (table) => {
...
async function updateEmailTemplate(req, res) {
    try {
        ...
        if (to !== undefined)
            updateData.to_field = to;
        if (from !== undefined)
            updateData.from_field = from;
        if (cc !== undefined)
            updateData.cc = cc;
        if (bcc !== undefined)
            updateData.bcc = bcc;
        ...
        await (0, database_1.db)('email_templates').where({ id }).update(updateData);
        const updated = await (0, database_1.db)('email_templates').where({ id }).first();
        res.json({
            data: {
                id: updated.id,
                name: updated.name,
...
async function deleteEmailTemplate(req, res) {
    try {
        const { id } = req.params;
        await (0, database_1.db)('email_templates').where({ id }).delete();
